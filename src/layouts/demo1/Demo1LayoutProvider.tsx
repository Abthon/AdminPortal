...
import { deepMerge } from '@/utils';
import { demo1LayoutConfig } from './';
// Import auth context to resolve the menu config function
...
const Demo1LayoutProvider = ({ children }: PropsWithChildren) => {
  ...
  // Resolve the menu configuration by calling MENU_SIDEBAR with the auth token and getUserType function.
  const { auth, getUserType } = useAuthContext();
  const primaryMenu = MENU_SIDEBAR(auth?.accessToken || '', getUserType);
  const secondaryMenu = useMenuChildren(pathname, primaryMenu, 0); // Now primaryMenu is an array
