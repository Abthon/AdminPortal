...
const SidebarMenu = () => {
  ...
  // Defensive buildMenu: ensure items is an array.
  const buildMenu = (items: TMenuConfig) => {
    if (!Array.isArray(items)) {
      console.error('buildMenu expects an array but got:', items);
      return null;
    }
  ...
  const buildMenuItemChildren = (items: TMenuConfig, index: number, level: number = 0) => {
    if (!Array.isArray(items)) {
      console.error('buildMenuItemChildren expects an array but got:', items);
      return null;
    }
    return items.map((item, index) => {
      if (item.disabled) {
  ...
  const { getMenuConfig } = useMenus();
  // Get the menu config for "primary"
  const menuConfigFunction = getMenuConfig('primary') as MenuConfigType;
  const { auth, getUserType } = useAuthContext();
  const menuConfig =
    typeof menuConfigFunction === 'function'
      ? menuConfigFunction(auth?.accessToken || '', getUserType)
      : menuConfigFunction;
