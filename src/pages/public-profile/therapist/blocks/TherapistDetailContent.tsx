...
const TherapistDetailContent = ({ therapistData }: TherapistDetailContentProps) => {
  ...
  const profileImage = therapistData.profile 
    ...
    console.log(therapistData.rating, "gow gow rating");
...
const TherapistStatistics = ({ therapistData }: { therapistData: ITherapistDetailData }) => {
  const profileImage = therapistData.profile 
    ? `${BASE_URL}/${therapistData.profile}` 
    : avatar;
        ...
        <div className="flex flex-col items-center text-center py-8">
          ...
          <div className="mb-4">
            <div className="flex items-center justify-center gap-2 mb-2">
              ...
              {(therapistData.isEmailAuthenticated || therapistData.isPhoneNumberAuthenticated) && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 py-4 border-t">
          <div className="text-center">
            <div className="text-3xl font-bold text-gray-900 mb-1">
              {(() => {
                if (!therapistData.dob) return 'N/A';
                const dobDate = new Date(therapistData.dob);
          ...
          <div className="text-center">
            <div className="text-3xl font-bold text-gray-900 mb-1">
              {therapistData.isEmailAuthenticated && therapistData.isPhoneNumberAuthenticated ? "2" : 
               therapistData.isEmailAuthenticated || therapistData.isPhoneNumberAuthenticated ? "1" : "0"}
            </div>
...
const TherapistGeneralInfo = ({ therapistData }: { therapistData: ITherapistDetailData }) => {
  ...
  const items = [
    ...
    { 
      ...
      info: `<span class="badge badge-sm ${
        therapistData.status === "suspended" ? "badge-danger" : 
        therapistData.status === "inactive" ? "badge-warning" : 
        therapistData.status === "active" ? "badge-success" : 
        "badge-primary"
      } badge-outline">${therapistData.status}</span>` 
    },
    { label: "Gender:", info: therapistData.gender },
    { label: "Hours Dedicated Per Week:", info: `${therapistData.hoursDedicatedPerWeek}` },
...
const TherapistAccountInfo = ({ therapistData }: { therapistData: ITherapistDetailData }) => {
  const items = [
    { label: "User ID:", info: therapistData.id },
    { label: "Date of Birth:", info: therapistData.dob ? (() => {
    ...
    { label: "Visibility:", info: therapistData.isVisible ? "Visible" : "Hidden" },
...
const TherapistTabbedContent = ({ therapistData }: { therapistData: ITherapistDetailData }) => {
  ...
  return (
    <div className="flex flex-col gap-5 lg:gap-7.5">
      ...
      {/* Tab Content */}
      {activeView === "sessions" && <TherapistSessions therapistData={therapistData} />}
      {activeView === "activity" && <TherapistActivity therapistData={therapistData} />}
      {activeView === "ratings" && <TherapistRatings therapistData={therapistData} />}
    </div>
  );
};

// Sessions Component
const TherapistSessions = ({ therapistData }: { therapistData: ITherapistDetailData }) => {
  ...
  const fetchSessions = async (): Promise<ISessionResponse> => {
    const { data } = await axiosInstance.get(`/api/v1/session?fields=client.*,schedule&filters=therapist.id=${therapistData.id}`);
    return data;
  };
...
// Activity Component
const TherapistActivity = ({ therapistData }: { therapistData: ITherapistDetailData }) => {
  return (
    <div className="card">
      <div className="card-header">
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
            ...
            <div>
              ...
              <div className="text-xs text-gray-600">{timeAgo(therapistData.createdAt)}</div>
            </div>
          </div>
          <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
            ...
            <div>
              ...
              <div className="text-xs text-gray-600">{timeAgo(therapistData.updatedAt)}</div>
            </div>
          </div>
          {therapistData.lastSeenAt && (
            <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
              ...
              <div>
                ...
                <div className="text-xs text-gray-600">{timeAgo(therapistData.lastSeenAt)}</div>
              </div>
            </div>
          )}
        </div>
...
// Ratings Component
const TherapistRatings = ({ therapistData }: { therapistData: ITherapistDetailData }) => {
  const ratings = therapistData.rating || [];
  
  const calculateAverageRating = () => {
    if (ratings.length === 0) return "0.0";
    const sum = ratings.reduce((acc: number, rating: ITherapistRating) => acc + rating.value, 0);
      ...
      <div className="card-body">
        {ratings.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-gray-600">This therapist hasn't received any ratings.</p>
          </div>
        ) : (
          <div className="space-y-6">
            ...
            <div className="flex items-center gap-6 p-4 bg-gray-50 rounded-lg">
              ...
              <div className="flex-1">
                ...
                {Object.entries(distribution).reverse().map(([star, count]) => (
            ...
            {/* Ratings Table */}
            <RatingsTable ratings={ratings} renderStars={renderStars} timeAgo={timeAgo} />
          </div>
        )}
      </div>
    </div>
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <span className="text-sm text-gray-600">
          Showing {startIndex + 1}-{Math.min(endIndex, ratings.length)} of {ratings.length} ratings
        </span>
      </div>
        ...
        <table className="w-full border-collapse border border-gray-200 rounded-lg">
          ...
          <tbody>
            {currentRatings.map((rating, index) => (
              <tr key={rating.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
        ...
        <div className="mt-6 p-4 bg-gray-50 rounded-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-900">{timeAgo(session.client.lastSeenAt)}</p>
            </div>
          </div>
        </div>
      </div>
