...
const TherapistDetailPage = () => {
  ...
  const fetchTherapistDetail = async (therapistId: string): Promise<ITherapistDetailResponse> => {
    try {
      const { data } = await axiosInstance.get(`/api/v1/therapist/${therapistId}?fields=rating.*,email,gender,createdAt,phoneNumber,status,profile,hoursDedicatedPerWeek,bio,expertise.*,therapistBank.*,firstName,lastName`);
      return data;
    } catch (error: any) {
