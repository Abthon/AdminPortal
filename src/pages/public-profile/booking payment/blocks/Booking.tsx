...
const Booking: React.FC<BookingProps> = ({
  ...
  async function searchBooking({
    ...
    const url = `/api/v1/bookings?filters=pickupname=${search},status=completed,paymentMethod=${activeTab == "bookings" ? "null" : activeTab == "cash" ? "cash" : "invoice"}&take=${pageSize}&page=${pageIndex}&sort=createdAt=${sort[0].desc ? "DESC" : "ASC"}`;
    const { data } = await axiosInstance.get(url);
    // calculating how many items are there on the current page
    const startIndex =
      (data.pagination.currentPage - 1) * data.pagination.pageSize + 1;
  ...
  const changePayment = async ({
    ...
    await axiosInstance.patch(`/api/v1/bookings/${id}/payment-method`, {
      paymentMethod: method,
    });
  };

  let { isLoading: isBookingLoading, data: BookingData } = useQuery({
    queryKey: ["Bookings", searchInput, activeTab],
  ...
  const { isLoading: isDeleting, mutate } = useMutation({
    mutationFn: changePayment,
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["Bookings"],
      });
      toast("Booking successfully paid!");
    },
    onError: (error: unknown) => {
      toast("Error Encountered paying the booking");
    },
  });
