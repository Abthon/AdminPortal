...
const approvalClientSchema = Yup.object().shape({
  approvalStatus: Yup.string().required("Approval status is required."),
});

interface IModalClientTypeFormProps {
  open: boolean;
  isEdit: boolean;
  isApproved: boolean;
  isDelete?: boolean;
...
const ModalClientTypeForm = ({
  ...
  isDelete,
  clientData,
}: IModalClientTypeFormProps) => {
  const [showPassword, setShowPassword] = useState(false);
  const queryClient = useQueryClient();
  const { mutate } = useMutation({
    mutationFn: isApproved ? approveClient : isEdit ? editClient : addClient,
    onSuccess: () => {
      console.log("Mutation called with:", { isEdit, isApproved });
      toast.success(
        `${isApproved ? "Client Approved" : isEdit ? "Client Edited" : "Client Created"}`
      );
  ...
  async function approveClient(values: Object) {
    console.log("Approve client values:", values, "is called!");
    try {
      ...
      const res = await axiosInstance.patch(
        `/api/v1/client/toggleStatus/${clientData.id}`,
        updatedFields
      );
      return res.data;
    } catch (err: any) {
  ...
  async function deleteClient(id: any) {
    try {
      const res = await axiosInstance.delete(`/api/v1/client/${id}`);
      if (res.status !== 200) {
        throw new Error(res.data.message || "Failed to delete the client.");
      }
  ...
  async function addClient(values: any) {
    try {
      console.log("Adding client with values:", values);
  ...
  async function editClient(values: any) {
    try {
      ...
      const errorMessage =
        err?.response?.data?.message ||
        "An error occurred while editing the client.";
      throw new Error(errorMessage);
    }
  }

  const initialValues = isApproved
    ? { approvalStatus: clientData?.status || "pending" }
    : isEdit
      ? {
          ...clientData,
  ...
  useEffect(() => {
    if (open) {
      if (isApproved) {
        console.log("Setting form values for approval:", clientData);
        formik.setValues({
          approvalStatus: clientData?.status || "pending",
        });
      } else if (isEdit) {
        console.log("Setting form values for edit:", clientData);
        formik.setValues({
          ...clientData,
          password: "", // Keep password empty for security
          phoneNumber: clientData.phoneNumber || "",
          ...
          email: clientData.email || "",
          dob: clientData.dob
            ? new Date(clientData.dob).toISOString().split("T")[0]
            : "",
        });
      } else {
        formik.resetForm();
  ...
  }, [isEdit, isApproved, open, clientData]);
